...
if platform.machine() == "x86_64":
    EXTRA_COMPILE_ARGS += ["-mno-avx", "-mno-avx2"]
...
def build_zlib():
    ...
    cc = new_compiler()
    return cc.compile(
        sources, include_dirs=include_dirs, extra_preargs=extra_compile_args
    )
...
def build_zstd():
    ...
    return (
        cc.compile(sources, include_dirs=include_dirs, extra_preargs=extra_compile_args)
        + compiled
    )
